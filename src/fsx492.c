...
ctx->inodes[dir_ino].direct_blks[i] = blockn;
...
void *fsx492_init(struct fuse_conn_info *conn, struct fuse_config *cfg)
{
    ...
    _ctx->block_map = (fd_set *)((uint8_t *)_ctx->metadata + (1 + sb.inode_map_sz) * FSX492_BLKSZ);
    _ctx->inodes = (struct fsx492_inode *)((uint8_t *)_ctx->metadata + (1 + sb.inode_map_sz + sb.block_map_sz) * FSX492_BLKSZ);
                ...
                off_t offset, struct fuse_file_info *fi)
{
    ...
    if (to_read > 0 && offset < indir2_sz)
    {
        ...
        for (size_t i = start_i; to_read > 0 && i < FSX492_PTRS_PER_BLK; i++)
        {
            ...
            // compute starting data block in indir1 block
            const size_t start_j = (offset / FSX492_BLKSZ - FSX492_N_DIRECT - FSX492_PTRS_PER_BLK) % FSX492_PTRS_PER_BLK;
...
int fsx492_release(const char *path, struct fuse_file_info *fi)
{
    ...
    // write back metadata
    if(writeback_metadata((struct context *)fuse_get_context()->private_data) < 0){
        return -EIO;
    }
    return 0;
}
...
int fsx492_mkdir(const char *path, mode_t mode)
{
    ...
    uint32_t target_ino, parent_ino, new_ino;
    int ret = 0;
    //how to get/define target/parent?
...
int fsx492_opendir(const char *path, struct fuse_file_info *fi)
{
    ...
    struct fh *dir = malloc(sizeof(struct fh));
    if(!dir){
        return -ENOSPC;
    }
    dir->ino = ino;
...
int fsx492_rmdir(const char *path)
{
    ...
    struct fsx492_dirent buf[FSX492_DIRENTRIES_PER_BLK];
    for(int i=0; i < FSX492_N_DIRECT; i++){
        if(ctx->inodes[ino].direct_blks[i] == 0){
            continue;
        }
        if(read_blks(ctx->inodes[ino].direct_blks[i], 1, buf) < 0){
            return -EIO;
        }
        for(int j = 0; j < FSX492_DIRENTRIES_PER_BLK; j++){
            if(!buf[j].valid){
            ...
            if(strcmp(buf[j].name, ".") != 0 && strcmp(buf[j].name, "..") != 0){
                return -ENOTEMPTY;
            }
        }
    }
